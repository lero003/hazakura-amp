...
- [ ] **リセット**: ゲインを 1.0 へ戻すと、**1 秒以内に** 出力音がニュートラルへ戻る
...
- [ ] Activity Monitor から "Force Quit" で終了した直後、**tap / aggregate device / routing が OS 側にゴミとして残らない**（`system_profiler SPAudioDataType` などで確認）
...
# 5. 100% / 200% / 400% のスライダーまたは固定値でゲインを切り替え
# 6. 終了 → 再起動シナリオを確認
```
