...
> 関連: [企画書 §音割れ / §やらないこと](../hazakura-volume-booster企画書.md) / [ARCHITECTURE](./ARCHITECTURE.md) / [PERMISSIONS](./PERMISSIONS.md) / [ROADMAP](./ROADMAP.md)

Hazakura Boostは、表面上は「スライダー1本のシンプルアプリ」に見えるが、根幹には**Macのシステムオーディオをリアルタイムに加工する**という落とし穴だらけの領域がある。このドキュメントでは、実装に踏み出す前に対処法と撤退ラインを決めておきたいリスクを列挙する。
...
- 縮退案として「**自プロセスが再生する音声のみ**にブースト」もあるが、これは **Hazakura Boost の価値提案（YouTubeやシステム音を持ち上げる）を毀損する**ため MVP ではなく **技術デモ扱い**とし、製品リリースとしては採用しない
...
**撤退ライン**:
- App Store 配布を諦めてもよい（Hazakura Boostは小さなユーティリティで、App Store審査のコストメリットが小さい可能性がある）
...
**問題**: アプリが落ちたりユーザーが `⌘Q` で終了したときに、**最後に設定されたゲインが出力に固着**してしまうと、終了後に過大音量が出続けてしまう。これは Hazakura Boost が直接的に引き起こし得る最悪の不具合。
...
**問題**: メニューバーにごちゃっと他の常駐アプリがいると、Hazakura Boost のアイコンが埋もれて、**「いまONなのかOFFなのか、Boostが効いているのか」**が一目でわからない。
