...
Hazakura Ampは、表面上は「スライダー1本のシンプルアプリ」に見えるが、根幹には**Macのシステムオーディオをリアルタイムに加工する**という落とし穴だらけの領域がある。このドキュメントでは、実装に踏み出す前に対処法と撤退ラインを決めておきたいリスクを列挙する。
...
**撤退ライン**:
- App Store 配布を諦めてもよい（Hazakura Ampは小さなユーティリティで、App Store審査のコストメリットが小さい可能性がある）
...
### 10a. 🟠 Safari 拡張 + zip 配布で署名が壊れる（SFErrorDomain:1）

**問題**: `ditto -c -k --keepParent` だけで zip すると、`com.apple.provenance` 等の xattr が AppleDouble（`._*`）として zip に入る。Finder の Archive Utility / `ditto -x` は xattr に戻すが、**`unzip` や一部展開経路は `._*` を実ファイルとして書き出し**、コード署名の sealed resources が壊れる。結果として Safari が埋め込み Web Extension を認識できず `SFErrorDomain:1` になり、「未署名拡張を許可」でも安定しない。

**対策**:
- `package_zip` で `--norsrc --noextattr` + `xattr -cr` してから zip する
- パッケージ直後に **plain `unzip` + `codesign --verify --strict --deep`** を回帰チェックする
- インストールは `/Applications` に置き、アプリを一度起動してから Safari 拡張を有効化する
- 未署名拡張の Develop メニュー許可は開発用の一時回避であり、Safari 終了でリセットされる。正規配布は Developer ID（ ideally notarized）を使う
...
| 10a | 🟠 | zip の AppleDouble で Safari 拡張が死ぬ | `package_zip` で xattr 除去 + plain unzip 検証 |
