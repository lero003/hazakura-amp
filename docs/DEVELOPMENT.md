...
Hazakura Boostの開発に参加する人（自分自身も含む）とAIエージェントが、**最短で開発環境を整え、ビルドし、テストし、リリース前段まで進められる**ようにするためのドキュメントです。
...
```bash
# 1. リポジトリを取得
git clone <repository-url> hazakura-volume-booster
cd hazakura-volume-booster
...
# 3. Xcodeでプロジェクトを開く
#    （v0.1 着手時に hazakura-volume-booster.xcodeproj を作成する）
open hazakura-volume-booster.xcodeproj
```
...
# コマンドラインからクリーンビルド
xcodebuild \
  -project hazakura-volume-booster.xcodeproj \
  -scheme hazakura-volume-booster \
  -configuration Debug \
...
```
~/Library/Developer/Xcode/DerivedData/hazakura-volume-booster-*/Build/Products/Debug/Hazakura Boost.app
```
...
- **ロギング**: `os.Logger` / `OSLog` を使い、サブシステムを `dev.keisetsu.hazakura-volume-booster` で統一
...
xcodebuild \
  ...
  -archivePath build/HazakuraBoost.xcarchive \
  archive
