...
Hazakura Ampの中核は、**「Macのシステム音を一時的にタップし、ゲインを掛けてから出力へ戻す」**という一本のパイプラインです。Macの音量をソフトウェアから持ち上げるには、原則としてこの「タップ → ゲイン → 出力」のどこかに割り込む必要があります。

このドキュメントは、v0.1〜v0.2を視野に入れたシステム全体のアーキ・主要コンポーネント・データフロー・技術選定の理由を整理します。

> 現在の active 実装は `spike/core-audio-tap` の ScreenCaptureKit + ring buffer + AVAudioEngine 経路です。v0.2候補では、UIの `Reset` / preset / ON-OFF を削除し、状態は `configuredGain` と start/stop に寄せています。
...
│   │  - state: BoostState { configuredGain, isRunning }  │   │
│   │  - setGain / start / stop / shutdown                 │   │
...
役割:
- 現在の `BoostState` を保持（configuredGain と isRunning）
- UI ↔ Audio Engineの橋渡し
- ライフサイクル管理（起動/終了時の安全な停止）
- 設定の永続化（UserDefaults への debounce 付き書き込み）

`BoostState` は表示用の `displayPercent` と出力用の `effectiveGain` を導出するだけの不変量に近い構造体とし、**「Slider値は停止中も保持するが、停止中の出力は 1.0 に固定する」** という仕様を型で表現する。
...
struct BoostState: Equatable {
    ...
    /// 実行中のみ configuredGain を反映。停止中は 1.0 固定。
    var isRunning: Bool

    /// オーディオ出力に乗せる実効ゲイン
    var effectiveGain: Double { isRunning ? configuredGain : 1.0 }

    /// UI表示用（例: 100, 180, 300）。100 のとき "100%"、それ以外は "Boost NN%"。
    var displayPercent: Int { Int((configuredGain * 100).rounded()) }

    static let neutral = BoostState(configuredGain: 1.0, isRunning: false)
}
```
...
HazakuraAmpApp.init()
   ...
   └─▶ controller.loadFromSettings()         // configuredGain を復元
...
### ユーザーがSliderを100%へ戻したとき

```
BoostSliderView
   └─▶ controller.setGain(1.0)
...
StopButton
   └─▶ controller.stop()
         ├─▶ AudioEngine.applyGain(1.0)
         ├─▶ AudioEngine.stop()
         └─▶ state.isRunning = false          // configuredGain は保持
```

> 停止中は `state.configuredGain` を保持したまま、AudioEngine への適用値だけ `effectiveGain = 1.0` に切り替える。Start すると `configuredGain` へ復帰する。
...
スリープ前
   ...
   復帰時
   └─▶ didWakeNotification
         └─▶ controller.restoreFromSettings()  // configuredGain を復元
```
...
1. **`effectiveGain = 1.0` が「ニュートラル」**: システム音は `effectiveGain == 1.0` のとき限りなく素通しに近くあること。`configuredGain == 1.0` または `isRunning == false` のいずれか一方でも満たせば成立。
...
4. **状態は `BoostController.state` を単一のソース・オブ・トゥルースとする**: ビュー側で `configuredGain` / `isRunning` を別途保持しない。表示用には `displayPercent` 導出プロパティを使う。
...
- [ ] **スリープ/復帰時の挙動**: スリープ前に gain=1.0 → stop（configuredGain は保持）、復帰時は自動復元または手動 Start で保存値へ戻す。
