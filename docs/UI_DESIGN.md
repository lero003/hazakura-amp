...
アイコンの形（塗りなし/塗りあり）だけで ON/OFF を明示する。%ラベルは文字数変動でアイコン位置がガタつくため採用しない。細かい数値はポップオーバー内の Boost ラベルで確認する。

| 状態 | アイコン | 意図 |
|---|---|---|
| 動作中（0%〜200%） | `speaker.wave.2.fill` | ブースト中。塗りありで動いていることが一目でわかる |
| 動作中（201%〜400%） | `speaker.wave.3.fill` | 強ブースト中。波を1つ増やして強さを示す |
| 停止中 | `speaker.wave.2` | 音声経路が動いていない。塗りなしで控えめ |

ホバー toolTip は状態チップと同じ語彙（動作中 / 停止中 / 権限が必要 など）を使い、動作中のみ % を添える。

> 実装は `RightClickableStatusButton` が `PoCAudioEngine` の `isRunning` / `configuredGain` を Combine で購読し、SF Symbol を切り替える。`statusItem` の幅は `squareLength`（固定）で、状態が変わってもメニューバーのアイコン位置が動かない。
...
- **幅 380pt**、高さは内容可変（最大 560pt。Dev 診断のイベントログは 160pt 上限でスクロール）
...
```
┌──────────────────────────────────┐
│ Hazakura Amp        [● 動作中]   │ ← Header + status chip
│ 動作中                           │
│ オーディオはローカルで…           │
│                                  │
│ ブースト              ブースト 180%│ ← ラベル（右寄せ・等幅）
│ ┌──────────────────────────────┐ │
│ │  ●─────────────              │ │ ← Slider（0%〜400%）
│ └──────────────────────────────┘ │
│ [標準] [動画] [講義] [最大]       │ ← プリセット
│ （300%以上は短い注意文）          │
│                                  │
│ 音質  低/中/高 ±6 dB             │ ← 簡易3バンドEQ
│                                  │
│ [ 開始 / 停止 ]          [ 終了 ] │ ← 開始は prominent
│                                  │
│ ▸ 診断                           │ ← DisclosureGroup（既定閉じ）
│ ローカル処理のみ · 録音なし  v0.4.0│
└──────────────────────────────────┘
```

> v0.2 では Reset / ON-OFF を削った。v0.4 ではロードマップどおり **固定プリセット 4 つ** と **簡易3バンドEQ** を戻している。実装UI文字列は日本語が正（§8）。内部状態と診断コピー（クリップボード送信）は英語寄りで安定させる。

余白:
- 左右パディング: 16pt
- 要素間: 12–14pt
- セクション区切り: 罫線を使わず、空白でリズムを作る
- メニューバーアイコン: ホバーで toolTip（例: `Hazakura Amp — 動作中 180%`）
...
- ポップオーバー内に SF Symbol 付きバナー（絵文字は使わない）: 再試行を促す文言
- 主操作の「開始」を prominent のまま残し、視線を再試行へ誘導する
- 詳細は診断 DisclosureGroup で確認できる
...
- v0.2 の **実装UI文字列は日本語を正**とする。ボタン・見出し・診断ラベル・状態メッセージは日本語で実装する
- `Info.plist` の `NSAudioCaptureUsageDescription` 等は OS ダイアログ向けのため **英語を正** とする（システムロケールに合わせる）
- **VoiceOver / accessibility ラベルは英語を正**とする（`accessibilityLabel` / `accessibilityHint` は英語ベース）。表示文言の日本語化と、スクリーンリーダー読み上げの英語ベースは分離して管理する
- このドキュメント内の英語UIテキストは **設計説明用の補足** に留め、**実装上の正本は日本語** として扱う
  - ポップオーバーヘッダ: "Hazakura Amp"（ブランド名のため英語のまま）
  - ボタンの実装値:
    - 開始/停止: 「開始」/「停止」（設計メモ: 英語UI案 = `Start` / `Stop`）
    - 終了: 「終了」（設計メモ: 英語UI案 = `Quit`）
  - エラーバナー: SF Symbol + 「システム設定 > プライバシーとセキュリティ で…」案内（絵文字は使わない）
  - 起動時メッセージ: 「ブーストを開始してください / システム音をローカル処理します。録音・保存・送信は行いません。」
  - 診断: 既定は折りたたみ。accessibility ラベルは `Developer diagnostics`
- 文字列は現状リテラル直書き。i18n（`Localizable.strings` / `String(localized:)`）は英/日 2言語化のタイミングで導入する
