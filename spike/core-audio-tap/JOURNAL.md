...
- **3 つの異なるアプローチを順に試したが、PoC 段階（システム音のタップ → 加工 → ループバック出力）が未完成**
- 計画ドキュメント（`docs/` 配下 8 本）は完成済み、レビュー反映済み
- 環境前提（macOS 26+、NSAudioCaptureUsageDescription、HALOutput 等の Audio Toolbox API）は整っている
- **次の一手は「リングバッファ + AVAudioEngine 出力」アーキに切り替える**のが最有力
- 詳細は後述の「v0/v1/v2 試行」「次の仮説」「再開手順」を参照
...
| 作業ディレクトリ | `/Users/keisetsu/Projects/hazakura-volume-booster/spike/core-audio-tap/` |
...
| `spike/core-audio-tap/README.md` | ✅ 旧 AVAudioEngine ベースの説明になっている。v2 構成移行後に要更新 |
...
│   ├── CoreAudioTapPoC-Bridging-Header.h  (→ AudioIOProc.h、未使用 import 残置)
│   ├── Audio/
│   │   ├── SystemTap.swift              CATapDescription + Aggregate device
│   │   ├── AudioIOProc.h / .mm          ★ 残置（v1 アプローチのコード、未使用）
│   │   ├── AudioUnitInput.swift         ★ v2 の中核、active な実装
│   │   ├── GainProcessor.swift          linear→dB の数式ヘルパ + 単体テスト
...
└── CoreAudioTapPoCTests/
    └── GainProcessorTests.swift        4 tests, 全て pass（linear→dB 数式）
```
...
| ファイル | 状態 | 備考 |
|---|---|---|
| `SystemTap.swift` | ✅ 動作確認済み（tap 作成・aggregate 作成で noErr） | `muteBehavior = .unmuted`（デバッグ用）、`kAudioAggregateDeviceIsPrivateKey: true`、sub-device に default output を含む |
| `AudioUnitInput.swift` | ⚠️ v2 構成で CurrentDevice failed | order: EnableIO → CurrentDevice → StreamFormat → SetRenderCallback → Init → Start |
| `PoCAudioEngine.swift` | ✅ 動作確認済み | AudioUnit ベースに切替済み、診断タイマーで `ioProcCallCount` / `lastObservedGain` を UI に表示 |
| `ContentView.swift` | ✅ 動作確認済み | `DiagnosticsView` を含む |
| `AudioIOProc.h/.mm` | ❌ 未使用、削除推奨 | Bridging Header 経由で import されているが、実コードからは呼ばれていない |
| `GainProcessor.swift` | ✅ 動作確認済み | IO proc 自体は linear gain を直接扱うので runtime 未使用。テスト用に保持 |
| `Info.plist` | ✅ 動作確認済み | `LSUIElement = false`（PoC は window あり） |
...
### 仮説 A: ring buffer + AVAudioEngine 出力

aggregate からは **入力専用**で PCM を受け取り、**ring buffer に書き込む**。出力は別経路で、**AVAudioEngine + カスタム `AVAudioSourceNode`** が ring buffer を読んで default device へ出力する。
...
**PoC で確認すること**:
- `AVAudioSourceNode` から ring buffer を読んで Float32 stereo で出力
- ring buffer は SPSC (single-producer single-consumer) で十分
- 試しに `kAudioAggregateDeviceIsPrivateKey: false` でまず入力側を成立させてから、出力ルートを AVAudioEngine 化

### 仮説 B: aggregate を non-private にしてみる
...
```bash
cd /Users/keisetsu/Projects/hazakura-volume-booster/spike/core-audio-tap
...
→ v0.1 実装を保留。ScreenCaptureKit / 別プロダクト化 / Apple への Feedback Assistant 提出 を再評価。
