...
final class GainProcessorTests: XCTestCase {
    ...
    func testInfoPlistUsesV041ReleaseVersion() throws {
        let plist = try loadInfoPlist()

        XCTAssertEqual(plist["CFBundleShortVersionString"] as? String, "0.4.1")
    ...
    func testContentViewSourceKeepsOnlyEssentialBoostControls() throws {
        ...
        XCTAssertTrue(source.contains("Button(\"停止\", action: onToggle)"))
        XCTAssertTrue(source.contains(".buttonStyle(.bordered)"))
        XCTAssertFalse(source.contains(".tint(isRunning ? Color.secondary"))
    ...
    func testYouTubeRemoteContentScriptUsesOnlyRepeatAndRemoteControls() throws {
        ...
        XCTAssertTrue(source.contains("setCaptionsViaPlayerApi"))
        XCTAssertTrue(source.contains("areCaptionsOn"))
        XCTAssertTrue(source.contains("simulateUserClick"))
        XCTAssertTrue(source.contains("setOption(\"captions\", \"track\""))
