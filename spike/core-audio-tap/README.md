...
注意: App Group `group.dev.keisetsu.hazakura-amp` を使うため、署名付き Debug/Release build には app と extension の bundle identifier に対応した provisioning profile が必要。profile がない環境では signed build は失敗するため、構成確認は上記の `CODE_SIGNING_ALLOWED=NO` build を使う。
...
- subsystem: `dev.keisetsu.hazakura-amp`
